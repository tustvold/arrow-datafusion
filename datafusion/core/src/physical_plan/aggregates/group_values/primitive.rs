...
impl<T: ArrowPrimitiveType> GroupValuesPrimitive<T> {
    pub fn new(data_type: DataType) -> Self {
        ...
        Self {
            ...
            map_size: 1024 * std::mem::size_of::<usize>(),
    ...
    T::Native: std::hash::Hash + Eq,
{
            ...
            let group_id = match v {
                ...
                Some(key) => {
                    ...
                    let insert = self.map.find_or_find_insert_slot(
                        hash,
                        |g| self.values[*g] == key,
                        |g| self.random_state.hash_one(self.values[*g]),
                    );

                    match insert {
                        Ok(v) => unsafe { *v.as_ref() },
                        Err(slot) => {
                            let g = self.values.len();
                            unsafe { self.map.insert_in_slot(hash, slot, g) };
    ...
    fn size(&self) -> usize {
        self.map_size + self.values.allocated_size()
    }
